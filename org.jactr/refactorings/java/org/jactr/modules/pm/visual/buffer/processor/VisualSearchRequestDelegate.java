...
public class VisualSearchRequestDelegate extends AbstractRequestDelegate
{
  ...
  @Override
  final protected Object startRequest(IRequest request, IActivationBuffer buffer, double requestTime)
  {
    ...
    return _module.scanVisualField(ctr, requestTime, isStuffRequest);
  }
      ...
      IActivationBuffer buffer, Object startValue)
  {
    ...
    if (error.equals(visualLocation) && !_isStuffRequest)
    {
    ...
    else
    {
      if (LOGGER.isDebugEnabled() || Logger.hasLoggers(model))
      {
        String msg = String.format("Found %1$s matching %2$s because of %3$s.", visualLocation,
            request, visualLocation.getMetaData(IPerceptualMemory.SEARCH_RESULT_IDENTIFIER_KEY));
        LOGGER.debug(msg);
        Logger.log(model, Logger.Stream.VISUAL, msg);
      }

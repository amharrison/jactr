    ...
    BasicAsynchronousModuleDelegate<AbstractVisualModule, PerceptualSearchResult>
{
      ...
      PerceptualSearchResult result, Object... parameters)
  {
    ...
    if (result == null && wasStuffRequest)
    {
      if (LOGGER.isDebugEnabled())
        LOGGER.debug("Stuff failed to find anything");
      return;
    }
    ...
    if (result == null)
    {
      if (LOGGER.isDebugEnabled())
      {
        String msg = "No valid visual location could be found matching requested "
            + searchPattern;
        LOGGER.debug(msg);
      }
    }
    ...
    if (module.hasListeners())
      module.dispatch(new VisualModuleEvent(module,
          VisualModuleEvent.Type.SEARCHED, location));
  }

...
@lexer::header{
package org.jactr.io.antlr3.parser.lisp2;
...
:	( clearAll | unknownList)* 
                 ...
                 */
                  model = support.createModelTree($name.text);
                 ...
                 (module)*
                 {
                  ...
                  try
                   {
                    //auto import 
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.declarative.six.DefaultDeclarativeModule6.class.getName(), true);
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.procedural.six.DefaultProceduralModule6.class.getName(), true);    
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.procedural.six.learning.DefaultProceduralLearningModule6.class.getName(), true);    
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.declarative.four.learning.DefaultDeclarativeLearningModule4.class.getName(), true);                    
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.imaginal.six.DefaultImaginalModule6.class.getName(), true);                                       
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.goal.six.DefaultGoalModule6.class.getName(), true);
                    _parser.getImportDelegate().importModuleInto($ModelGlobals::model, org.jactr.core.module.retrieval.six.DefaultRetrievalModule6.class.getName(), true);
			...
			*/
			...
			for(CommonTree chunk : $ModelGlobals::misplacedChunks)
			 reportError(new CompilationError(ASTSupport.getName(chunk)+" has no known chunktype" ,chunk));
...
module 	:	 OPEN_TOKEN 'module' s=string (i=NO_IMPORT_TOKEN)? CLOSE_TOKEN
               {
                 ...
                 try
                 {
                  _parser.getImportDelegate().importModuleInto($ModelGlobals::model, className, importContents);
...
@init{
 ...
 String className = null;
	...
	:	e=EQUALS_TOKEN name {isProxy="proxy".equalsIgnoreCase($name.text);}
	         GT_TOKEN b=bufferContent slots? 
	        -> {isProxy}? ^(PROXY_CONDITION[$e,"proxy"] CLASS_SPEC[((CommonTree)b.tree).token] slots?) 
	        -> ^(MATCH_CONDITION[$e,"match"] name bufferContent slots?);
...
@init{
 boolean isProxy = false;
	...
	:	 p=PLUS_TOKEN name {isProxy="proxy".equalsIgnoreCase($name.text);}
	         GT_TOKEN  (b=bufferContent)? slots? 
	         -> {isProxy}? ^(PROXY_ACTION[$p, "proxy"] CLASS_SPEC[((CommonTree)b.tree).token] slots?)
	         -> ^(ADD_ACTION[$p,"add"] name bufferContent? slots?);
/**
...
@init{boolean isOverwrite=false;}
	:	(e=EQUALS_TOKEN name GT_TOKEN) //common
	         ...
	         -> {isOverwrite}? ^(ADD_ACTION[$e,"add"] name var? identifier?) 
	         -> ^(MODIFY_ACTION[$e,"modify"] name slots?);
  finally{
    if(isOverwrite)
...
queryBuffer 
	:	q=QUESTION_TOKEN name GT_TOKEN slots? -> ^(QUERY_CONDITION[$q,"query"] name slots?);
	
removeBuffer 
	:	 m=MINUS_TOKEN name GT_TOKEN /*bufferContent slots?*/ ->^(REMOVE_ACTION[$m,"remove"] name /*bufferContent? slots?*/);
...
*/
bufferContent 
	:	(chunkIdentifier | isaType | var);

slotValue 	:	(identifier | var | number | string);
...
slot 	:	equalSlot | conditionalSlot;
...
@init{
 String rtn = null;
	...
	:	e=EQUALS_TOKEN v=IDENTIFIER_TOKEN
	{
	 rtn = $v.text;
	 rtn = "="+rtn;
	}  -> ^(VARIABLE[$v, rtn]); //not $v since its a return value
...
IDENTIFIER_TOKEN 
    : ( LETTER_TOKEN | PUNCTUATION_FRAGMENT) 
       (  LETTER_TOKEN | DIGITS_TOKEN | PUNCTUATION_FRAGMENT | '-' | '+')*
	;

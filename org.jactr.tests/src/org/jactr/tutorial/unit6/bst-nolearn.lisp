...
(defun update-current-line ()
  ...
  (if (= *current-stick* *target*)
      (progn
        (setf *done* t)
        (setf *current-line* (add-line-to-exp-window (list 50 135) (list  (+ *target* 50) 135) 'blue))
        (add-text-to-exp-window :x 180 :y 200 :width 50 :text "Done"))
    (if (zerop *current-stick*)
          (setf *current-line* nil)
        (setf *current-line* (add-line-to-exp-window (list 50 135) (list (+ *current-stick* 50) 135) 'blue))))
...
(sgp :v nil :esc t :pl nil :egs 3 :ut -100 
     :show-focus t)
